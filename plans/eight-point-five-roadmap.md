...
|---|---|---|---|---|
| 1.1 | **Audit and archive non-core features behind feature flags.** Create a single `config.features` map in `lib/config.ts` that toggles: `assetMarketplace`, `hypercerts`, `litProtocol` (legacy), `superrare`, `etherfuse`, `videoPipeline`. Default everything **off** except the core flow: Quick Games + IP Registration + CDR Vaults. | PREVENT BLOAT | 2h | ★★★★★ |
| 1.2 | **Resolve overlapping solutions.** CDR vaults have superseded Lit Protocol for secret panels. Remove Lit Protocol dependencies from the build (`@lit-protocol/*` packages) by migrating any remaining legacy decryption path to CDR-only. Update `config.ts` to remove the Lit toggle entirely. | CONSOLIDATION / ENHANCEMENT FIRST | 4h | ★★★★☆ |
...
| 5.5 | **Optimize the bundle.** The largest chunk is 2.4MB (likely `@rainbow-me/rainbowkit` + `wagmi` + `viem` + `@story-protocol/core-sdk` + `@lit-protocol/*`). Dynamic-import heavy SDKs: `@story-protocol/core-sdk` (only needed when user clicks IP registration), `@lit-protocol/*` (remove if superseded by CDR), `@piplabs/cdr-sdk` (only needed for secret panel unlock). Use `next/dynamic` with `ssr: false` for wallet-dependent components. | PERFORMANT / MODULAR | 4h | ★★★★☆ |
...
6. **1.2** — Remove Lit Protocol, finalize CDR-only
